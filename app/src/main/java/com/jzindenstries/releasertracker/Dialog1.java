...
public class Dialog1 {
    ...
    public void showDialog1(Activity activity, ArrayList<GoToAction> dataAction){
        ...
        Amount.setText("כמות מנתקים כוללת:  "+dataAction.size());
        ...
        for(GoToAction goToAction : dataAction){
            if(goToAction.getType().contains("FXC")){ counterType[0]++;}
            if(goToAction.getType().contains("5000")){ counterType[1]++;}
            if(goToAction.getType().contains("M1")){ counterType[2]++;}
            if(goToAction.getType().contains("M2")){ counterType[3]++;}
            if(goToAction.getType().contains("K35")){ counterType[4]++;}
        }
        pieArrayList();
